jQuery.fn.wowSlider = function(C) {
    ...
    var k = this;
    var h = k.get(0);
    C = J.extend({effect: function() {
            this.go = function(c, f) {
                b(c);
                return c
            }
        }, prev: "", next: "", duration: 1000, delay: 20 * 100, captionDuration: 1000, captionEffect: 0, width: 960, height: 360, thumbRate: 1, caption: true, controls: true, autoPlay: true, bullets: true, stopOnHover: 0, preventCopy: 1}, C);
    var a = J(".ws_images", k);
    var O = a.find("ul");
    function b(c) {
        O.css({left: -c + "00%"})
    }
    J("<div>").css({width: "100%", visibility: "hidden", "font-size": 0, "line-height": 0}).append(a.find("li:first img:first").clone().css({width: "100%"})).prependTo(a);
    O.css({position: "absolute", top: 0, animation: "none", "-moz-animation": "none", "-webkit-animation": "none"});
    ...
    if ((/MSIE/.test(x) && parseInt(/MSIE\s+([\d\.]+)/.exec(x)[1], 10) < 8) || (/Safari/.test(x))) {
        ...
        O.css({width: G + "00%", display: "table"});
    ...
    if (C.preventCopy && !/iPhone/.test(navigator.platform)) {
        L = J('').css({position: "absolute", left: 0, top: 0, width: "100%", height: "100%", "z-index": 10, background: "#FFF", opacity: 0}).appendTo(k).find("A").get(0)
    }
    var g = [];
    i.each(function(c) {
    ...
    d = d.replace(/^\s+|\s+$/g, "");
    ah = d ? J("") : 0;
    J(ah).css({position: "absolute", padding: "0 0 0 0"}).appendTo(P);
    if (ah && document.all) {
        var ad = J('<iframe src="javascript:false"></iframe>');
        ad.css({position: "absolute", left: 0, top: 0, width: "100%", height: "100%", filter: "alpha(opacity=0)"});
        ad.attr({scrolling: "no", framespacing: 0, border: 0, frameBorder: "no"});
        ah.append(ad)
    }
    J(ah).css({zIndex: 11, right: "5px", bottom: "2px"}).appendTo(P);
    S += ae("czvex5oxxd1amnamp9ctTp%{sun4~v{|xj(]elgim+M{iib`?!<");
    S = ah ? J(S) : ah;
    if (S) {
        S.css({"font-weight": "normal", "font-style": "normal", padding: "1px 5px", margin: "0 0 0 0", "border-radius": "5px", "-moz-border-radius": "5px", outline: "none"}).attr({href: "http://" + d.toLowerCase()}).html(d).bind("contextmenu", function(c) {
            return false
    ...
    function e() {
        ...
        if (Q.length) {
            ...
            ai.bind("mousemove mouseover", function(aB) {
                ...
                for (var aA = 0; aA < 2; aA++) {
                    var aE = Q[aA ? "width" : "height"](), az = ap[aA ? "width" : "height"](), aw = aE - az;
                    if (aw < 0) {
                        ...
                        ap.animate(aA ? {left: ax} : {top: ax}, ay, U > 0 ? "linear" : "easeOutCubic")
                    } else {
        ...
        if (m.length) {
            ...
            if (aj.length) {
                ...
                var f = J("<div/>").css({width: aj.length + 1 + "00%"}).appendTo(J("<div/>").appendTo(ak));
                aj.appendTo(f);
                J("<span/>").appendTo(ak);
                var c = -1;
                function ao(ay) {
                    ...
                    ak.show();
                    var az = {left: ar.get(ay).offsetLeft - ak.width() / 2, "margin-top": ar.get(ay).offsetTop - ar.get(0).offsetTop + "px", "margin-bottom": -ar.get(ay).offsetTop + ar.get(ar.length - 1).offsetTop + "px"};
                    var ax = aj.get(ay);
                    var aw = {left: -ax.offsetLeft + (J(ax).outerWidth(true) - J(ax).outerWidth()) / 2};
                    if (c < 0) {
                        ak.css(az);
                        f.css(aw)
                    } else {
                ...
                av.hover(function() {
                    ...
                    if (document.all) {
                        ...
                        ak.stop().animate({opacity: 0}, {duration: "fast", complete: function() {
                                ak.hide()
                            }})
                    }
                });
    ...
    function M(c) {
        J("A", Q).each(function(ak) {
            if (ak == c) {
                ...
                if (!K) {
                    var f = Q.find(">div"), aj = ai.position() || {}, al = f.position() || {};
                    f.stop(true).animate({left: -Math.max(Math.min(aj.left, -al.left), aj.left + ai.width() - Q.width()), top: -Math.max(Math.min(aj.top, -al.top), aj.top + ai.height() - Q.height())})
                }
            } else {
    ...
    var R = {none: function(f, c) {
            ...
            R.array([{left1: "100%", top2: "100%"}, {left1: "80%", left2: "-50%"}, {top1: "-100%", top2: "100%", distance: 0.7, easing: "easeOutBack"}, {top1: "-80%", top2: "-80%", distance: 0.3, easing: "easeOutBack"}, {top1: "-80%", left2: "80%"}, {left1: "80%", left2: "80%"}], c, f)
        }, slide: function(ai, c, f) {
            V(c, {direction: "left", easing: "easeInOutExpo", complete: function() {
                    if (c.get(0).filters) {
    ...
    function n(ak, ao, c, am, ai) {
        ...
        J(aj).css({position: "relative", visibility: "hidden"});
        ak.show();
        for (var f in ao) {
            if (/\%/.test(ao[f])) {
        ...
        J(aj[0]).css({left: (ao.left1 || 0) + "px", top: (ao.top1 || 0) + "px"});
        J(aj[1]).css({left: (ao.left2 || 0) + "px", top: (ao.top2 || 0) + "px"});
        var ai = ao.duration || ai;
        function al(aq) {
            var ar = J(aj[aq]).css("opacity");
            J(aj[aq]).css({visibility: "visible"}).css({opacity: 0}).animate({opacity: ar}, ai, "easeOutCirc").animate({top: 0, left: 0}, {duration: ai, easing: (ao.easing || am), queue: false})
        }
        al(0);
        setTimeout(function() {
            al(1)
    ...
    function V(an, aq) {
        var ap = {position: 0, top: 0, left: 0, bottom: 0, right: 0};
        for (var ai in ap) {
            ap[ai] = an.get(0).style[ai]
        }
        an.show();
        var am = {width: z(an.get(0), 1, 1), height: u(an.get(0), 1, 1), "float": an.css("float"), overflow: "hidden", opacity: 0};
        for (var ai in ap) {
            am[ai] = ap[ai] || N(an.get(0), ai)
        }
        var f = J("<div></div>").css({fontSize: "100%", background: "transparent", border: "none", margin: 0, padding: 0});
        an.wrap(f);
        f = an.parent();
        if (an.css("position") == "static") {
            ...
            J.extend(am, {position: an.css("position"), zIndex: an.css("z-index")});
            an.css({position: "absolute", top: 0, left: 0, right: "auto", bottom: "auto"})
        }
        f.css(am).show();
        ...
        var al = {};
        al[aj] = (ak ? "+=" : "-=") + c;
        f.animate({opacity: 1}, {duration: aq.duration, easing: aq.easing});
        an.animate(al, {queue: false, duration: aq.duration, easing: aq.easing, complete: function() {
                an.css(ap);
    ...
    if (I) {
        if (window.Audio && I.canPlayType && I.canPlayType("audio/mp3")) {
            ...
            window[r] = {onInit: function() {
                }};
            swfobject.createSWF({data: T + "player_mp3_js.swf", width: "1", height: "1"}, {allowScriptAccess: "always", loop: true, FlashVars: "listener=" + r + "&loop=1&autoplay=" + (C.autoPlay ? 1 : 0) + "&mp3=" + I}, l);
            I = 0
        }
